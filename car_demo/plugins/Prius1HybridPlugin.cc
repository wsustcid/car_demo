    ...
    : dataPtr(new PriusHybridPluginPrivate)
{
  int argc = 0;
  char *argv = nullptr;
  ros::init(argc, &argv, "Prius1HybridPlugin");
  ros::NodeHandle nh;
  this->dataPtr->controlSub = nh.subscribe("prius1", 10, &PriusHybridPlugin::OnPriusCommand, this);
...
void PriusHybridPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  this->dataPtr->node.Subscribe("/prius1/reset",
      &PriusHybridPlugin::OnReset, this);
  this->dataPtr->node.Subscribe("/prius1/stop",
      &PriusHybridPlugin::OnStop, this);

  this->dataPtr->node.Subscribe("/prius1/cmd_vel", &PriusHybridPlugin::OnCmdVel, this);
  this->dataPtr->node.Subscribe("/prius1/cmd_gear",
      &PriusHybridPlugin::OnCmdGear, this);
  this->dataPtr->node.Subscribe("/prius1/cmd_mode",
  ...
  this->dataPtr->consolePub =
    this->dataPtr->node.Advertise<ignition::msgs::Double_V>("/prius1/console");
  ...
  this->dataPtr->keyboardSub =
    this->dataPtr->gznode->Subscribe("~/prius1/keyboard/keypress",
        &PriusHybridPlugin::OnKeyPress, this, true);

  this->dataPtr->worldControlPub =
    this->dataPtr->gznode->Advertise<msgs::WorldControl>("~/prius1/world_control");

  this->dataPtr->node.Subscribe("/prius1/keypress", &PriusHybridPlugin::OnKeyPressIgn,
      this);
}
...
void PriusHybridPlugin::OnStop(const ignition::msgs::Any & /*_msg*/)
{
  ...
  bool executed = this->dataPtr->node.Request("/prius1cup/upload",
